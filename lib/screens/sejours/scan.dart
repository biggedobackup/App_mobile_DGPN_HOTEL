...
enum _ScanStep { initial, rectoDone }
...
class _ScanScreenState extends State<ScanScreen> {
  ...
  Future<void> _startScan() async {
    ...
    try {
      if (!await _documentReader.isReady) throw StateError('Scanner non prêt');
      ...
      setState(() {
        _isScanning = false;
        _status = 'Ouverture caméra impossible.';
      });
    }
  }
  ...
  Future<void> _handleCompletion(
    ...
    if (_step == _ScanStep.initial) {
      ...
      _openForm(finalData ?? _scanData!.copyWith(documentVerso: versoFile));
    }
  ...
  void _openForm(ScanResultData? data) {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (_) => EnregistrementScreen(scanData: data),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (data != null) ...[
            ...
            _row('Résidence', data.lieuResidence ?? '-'),
          ],
          const SizedBox(height: 16),
          Text(
            _status,
